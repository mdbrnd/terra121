# This branch includes my updated road generation algorithms. 
Instead of using RoadGenerator, I created a new class VectorRoadGenerator, which processes VectorPath types to create roads. VectorPaths store a bunch of x, y and z of locations that should have their BlockState changed in a CubicChunks Cube. The main reason, however, that I made the new class, is because of the other values it stores, mainly the relation String. That allows blocks to be checked for relation down to the 1x1x1. That means it will be far easier to generate structures that are OSM relation dependent, such as tunnels and bridges. When retured by the function used to create them using (almost) the same args as RoadGenerator.placeEdge(), they are in the form of a List of VectorPathGroup. VectorPathGroup simply stores an OpenStreetMaps.Type as well as a List of VectorPath. While having a seperate class for storing groups is pretty redundant right now, it could be used in the future for values that need to be passed over even at the general scale.
### This implementation is experimental. There will be (or is, idk if i'll end up updating the readme) a setting in with the rest of the settings called "Generate Advanced Roads", which will use the VectorRoad system instead of the RoadGenerator system. For the time being, I'll leave tunnels and bridges OFF in the standard road generation, because as far as I know, it isn't possible with the standard generation algorithm.
